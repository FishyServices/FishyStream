function normalizeAniListText(value) {
    return (value ?? "")
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, " ")
        .replace(/\s+/g, " ")
...
function getSeasonOrdinal(season) {
...
function extractTaggedSeasonNumbers(candidate, patterns, transform) {
...
function scoreAniListCandidate(media, title, season, year) {
    ...
    for (const candidate of titles) {
        ...
        const hasExplicitSeasonMismatch = explicitSeasonNumbers.size > 0 && !explicitSeasonNumbers.has(season);
        ...
        if (hasExplicitSeasonMatch)
            score += 14;
        if (hasExplicitSeasonMismatch)
            score -= 24;
        if (hasRomanSeasonMatch)
            score += 14;
        if (hasRomanSeasonMismatch)
            score -= 18;
        if (!hasExplicitSeasonMatch &&
            !hasExplicitSeasonMismatch &&
            !hasRomanSeasonMatch &&
            !hasRomanSeasonMismatch) {
            if (partNumbers.has(season))
                score += 4;
            if (courNumbers.has(season))
                score += 3;
        }
        else {
            if (partNumbers.has(season))
...
function parseRomanSeasonSignal(candidate, baseTitle) {
    if (!candidate.startsWith(baseTitle))
        return undefined;
...
async function searchAniListCandidate(search) {
    if (!search)
        return [];
    try {
...
async function fetchAniListYearCandidates(year, page) {
    try {
        const response = await fetch("https://graphql.anilist.co", {
            method: "POST",
            headers: {
...
async function resolveEpisodeInAniListChain(media, episode, visited) {
...
export async function resolveAniListEpisodeAddress(args) {
    ...
    return ((await resolveEpisodeInAniListChain(media, localEpisode, new Set())) ?? {
        anilistId,
