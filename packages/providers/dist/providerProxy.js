export const PROVIDER_PROXY_PREFIXES = ["vidplays-proxy"];
export const PROVIDER_PROXY_PATH_ALIASES = {
    vidplays: (segments) => ["vidplays-proxy", ...segments.slice(1)]
};
export function matchProviderProxyPath(pathname) {
    const normalized = pathname.replace(/^\/+/, "");
    const [prefix, ...rest] = normalized.split("/");
    if (!prefix)
        return null;
    if (!PROVIDER_PROXY_PREFIXES.some((candidate) => candidate === prefix))
        return null;
    return {
        prefix,
        subpath: rest.join("/")
...
export function resolveProviderProxyPathFromSegments(segments) {
...
export async function proxyProviderRequest({ url, method = "GET", headers = new Headers(), body }) {
...
async function proxyVidPlaysRequest({ url, subpath, method, headers = new Headers(), body }) {
    if (subpath === "hls") {
    ...
    if (subpath === "player.js") {
        ...
        return new Response(script.replaceAll('"/api/stream_data?type="', '"/vidplays-proxy/api/stream_data?type="'), {
    ...
    if (subpath.startsWith("embed/")) {
        ...
        return new Response(html
            .replaceAll('window.HOST = "https://vidplays.fun";', `window.HOST = "${url.origin}/vidplays-proxy";`)
...
async function proxyVidPlaysHls(url) {
...
function rewriteVidPlaysStreamUrls(value, origin) {
...
function toVidPlaysHlsProxyUrl(rawUrl, origin) {
    let target = rawUrl;
    try {
...
function rewriteHlsPlaylist(playlist, playlistUrl, origin) {
    return playlist
        .split("\n")
        .map((line) => {
