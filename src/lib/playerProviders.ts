...
export interface MediaDataPayload {
  type: "MEDIA_DATA";
  data: Record<string, VideasyMediaData>;
}
...
export function parsePlayerMessage(
  ...
  try {
    ...
    if (
      (p as PlayerEventPayload | MediaDataPayload).type === "MEDIA_DATA" &&
      !isVideasyOrigin(origin || "")
    ) {
      return p as MediaDataPayload;
    }

    if (isVideasyOrigin(origin || "") && (p as any).type === "MEDIA_DATA") {
      return parseVideasyMediaData(p);
    }

    if (isVideasyOrigin(origin || "") && isVideasyPayload(p)) {
      return {
        type: "PLAYER_EVENT",
        data: {
          event: "timeupdate",
